...
### langchain-community sunset (lab040, lab990)
- `langchain-community` is being sunset (github.com/langchain-ai/langchain-community/issues/674) and now prints a DeprecationWarning on import. lab040 `RAG_02.py` / `RAG_03.py` used it only for `TextLoader`, which just reads a file into a `Document`; replaced with `Document(page_content=Path(...).read_text(), metadata={"source": ...})` from `langchain_core`. Dropped `langchain-community` from lab040 `requirements.txt` and the dead `openai.api_key = ...` line from RAG_03. RAG_02 comment + README bullet explain the "chunk longer than specified" warnings (CharacterTextSplitter splits on blank lines only).
- lab990 `multi_turn.py` (+ doc): `ChatMessageHistory` -> `langchain_core.chat_history.InMemoryChatMessageHistory` (aliased, same API). `review_with_gitloader.py` still needs `langchain-community` for `GitLoader` (no core replacement); requirements comment updated.

### lab040_RAG (README: OpenAI-Beta header and Assistants sunset)
- Step 6 bullet claimed `OpenAI-Beta: assistants=v2` is needed for `/v1/files`. Never true: the official file-upload reference example has no beta header and the README's own step 2 curl does not send one. Checked 2026-09-18: the reference curl example for `/v1/vector_stores` still includes the header and openai-python 3.15.0 still injects it on every `resources/vector_stores/*` call (not on `files.py`, not on `responses/`). Kept the header in the vector store curl snippets, rewrote the three notes/bullets: header is an Assistants-era leftover, `/v1/files` and `/v1/responses` never needed it, students are invited to try a call without it. Also updated the "scheduled for deprecation in August 2026" wording: Assistants sunset happened 26 Aug 2026; vector stores + file_search continue under the Responses API. Not verified live whether the API now accepts vector store calls without the header.
- Follow-up, verified live on 2026-09-18: `/v1/vector_stores` calls succeed without `OpenAI-Beta: assistants=v2`. Removed the header from all Step 6 curl examples (create, add file x2, delete); notes rewritten to say it was removed and why. The SDK still sends it; harmless.
